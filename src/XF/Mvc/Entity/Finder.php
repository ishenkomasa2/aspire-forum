...
use function array_key_exists, array_slice, call_user_func_array, count, func_get_args, func_num_args, intval, is_array, is_int, is_null, is_string, strlen;
...
class Finder implements \IteratorAggregate
{
	...
	public function orderRandom($seed = null)
	{
		$this->resetOrder();
		return $this->writeSqlOrder('RAND(' . (is_null($seed) ? '' : intval($seed)) . ')');
	}
	...
	public function isOrderMatch($testOrder, $requireFullMatch = true)
	{
	...
	protected function renderToOrderSqlParts(array $orders)
	{
		$parts = [];
